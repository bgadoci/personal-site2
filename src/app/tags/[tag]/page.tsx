    ...
    <div className="space-y-8">
      ...
      {Object.entries(postsByCategory).map(([category, categoryPosts]) => (
        <div key={category} className="space-y-6">
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-50 capitalize flex items-center gap-2">
          ...
          <div className="grid gap-6">
            {categoryPosts.map((post) => (
              <div key={post.slug} className="relative group">
                ...
                {/* Card content */}
                <div className="bg-white dark:bg-slate-700 rounded-lg shadow-sm overflow-hidden border border-slate-200 dark:border-slate-700 p-6 group-hover:shadow-md group-hover:border-emerald-500 dark:group-hover:border-emerald-400 transition-all">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                    <div>
                      <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-50 mb-2">
                        <span className="group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">
                          {post.title}
                        </span>
                      </h3>
                    ...
                    <div className="text-emerald-500 dark:text-emerald-400 font-medium flex items-center">
                      {category === 'blog' && <DocumentTextIcon className="h-5 w-5" />}
                      {category === 'research' && <BeakerIcon className="h-5 w-5" />}
                      {category === 'book' && <BookOpenIcon className="h-5 w-5" />}
                      <span className="sr-only">Read {category === 'blog' ? 'article' : category === 'research' ? 'paper' : 'chapter'}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {post.tags.map((tag) => (
                      <TagLink
                        ...
                        className={`text-xs px-2 py-1 rounded transition-colors relative z-20 ${
                          tag.toLowerCase() === decodedTag.toLowerCase() 
                            ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' 
                            : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-600 dark:hover:text-emerald-400'
                        }`}
                      />
                    ))}
                  </div>
